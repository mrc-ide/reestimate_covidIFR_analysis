...
# pieces for below
studyidchar <- title <- toupper(stringr::str_split(basename(params$path), "_age|_rgn|_carehomes", simplify = T)[[1]])

# catch care homes
if (grepl("_carehomes", basename(params$path))) {
  title <- paste(title, "with Care Homes Excluded")
}

stratachar <- ifelse(grepl("age|carehomes", basename(params$path)), "Age-Band", "Region")
```
