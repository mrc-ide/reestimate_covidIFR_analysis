...
# read in data
#......................
modout <- readRDS("results/ModFits/BRA_age_GTI3_rung50_burn1000_smpl1000.RDS")
```
...
BRArcrudedat <- dscdat %>% 
  dplyr::filter(study_id == "BRA1" & breakdown == "ageband")  %>%
  dplyr::select(c("study_id", "plotdat")) %>%
  tidyr::unnest(cols = "plotdat")
ageplotdat <- BRArcrudedat %>%
...
# quick out
jpgsnapshot("figures/BRA_ifr_plot_age.jpg", plot = main_plotObj)
...
serocurvedat <- seropnts %>% 
  dplyr::select(c("chain", "sim", "ObsDay", dplyr::starts_with("RG_"), dplyr::starts_with("inf_"))) %>% 
...
jpgsnapshot("figures/BRA_ppc_plot_age.jpg", plot = ppc_plot_age)
...
# read in data
#......................
modout <- readRDS("results/ModFits/BRA_rgn_GTI3_rung50_burn1000_smpl1000.RDS")
```
...
dscdat <- readRDS("data/derived/descriptive_results_datamap.RDS") 
BRArcrudedat <- dscdat %>% 
  dplyr::filter(study_id == "BRA1" & breakdown == "region")  %>%
  dplyr::select(c("study_id", "plotdat")) %>%
  tidyr::unnest(cols = "plotdat")
rgnplotdat <- BRArcrudedat %>%
  dplyr::filter(seromidpt == obsday) %>%
  dplyr::mutate(infxns = popn * seroprev,
                crudeIFR =  cumdeaths/infxns) %>% 
...
# quick out
jpgsnapshot("figures/BRA_ifr_plot_rgn.jpg", plot = main_plotObj)
...
jpgsnapshot("figures/BRA_ppc_plot_rgn.jpg", plot = ppc_plot_rgn)
