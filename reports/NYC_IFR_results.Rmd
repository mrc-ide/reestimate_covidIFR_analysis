...
# read in data
#......................
modout <- readRDS("sandbox/nyc/temp3.RDS")
```
...
ageplotdat <- dscdat %>% 
  dplyr::filter(study_id == "NYC_NY_1" & breakdown == "ageband")  %>%
  dplyr::select(c("study_id", "plotdat")) %>%
  tidyr::unnest(cols = "plotdat")
ageplotdat <- ageplotdat %>%
...
jpeg("figures/NYC_ifr_plot_age.jpg", width = 11, height = 8, units = "in", res = 500)
main_plotObj
graphics.off()
...
##### Overall IFR
```{r, results='asis', fig.width=8, fig.height=6}
gIFR <- sum(ifrs$median * modout$inputs$IFRmodel$demog$popN/sum(modout$inputs$IFRmodel$demog$popN))
...
seropnts <- COVIDCurve::draw_posterior_sero_curves(IFRmodel_inf = modout,
                                                                  ...
                                                                  by_chain = FALSE)
# population denom
popdenom <- sum(modout$inputs$IFRmodel$demog$popN)

serocurvedat <- seropnts %>% 
  dplyr::select(c("sim", "seroprev")) %>% 
  ...
  dplyr::group_by_at(c("sim", "ObsDay", "seroprevlvl")) %>% 
...
jpeg("figures/NYC_ppc_plot_age.jpg", width = 11, height = 8, units = "in", res = 500)
ppc_plot_age
graphics.off()
...
<!-- # read in data -->
<!-- #...................... -->
<!-- modout <- readRDS("results/ModFits/NYC_rgn_GTI3_rung50_burn10000_smpl10000.RDS") -->
...
<!-- jpeg("figures/NYC_ifr_plot_rgn.jpg", width = 11, height = 8, units = "in", res = 500) -->
<!-- main_plotObj -->
<!-- graphics.off() -->
...
<!-- jpeg("figures/NYC_ppc_plot_rgn.jpg", width = 11, height = 8, units = "in", res = 500) -->
<!-- ppc_plot_rgn -->
<!-- graphics.off() -->
