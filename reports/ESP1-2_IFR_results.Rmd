...
# read in data
#......................
modout <- readRDS("results/ModFits/ESP_age_GTI2.5_rung50_burn1000_smpl1000.RDS")
...
ESPrcrudedat <- dscdat %>% 
  dplyr::filter(study_id == "ESP1" & breakdown == "ageband")  %>%
  dplyr::select(c("study_id", "plotdat")) %>%
  tidyr::unnest(cols = "plotdat")
ageplotdat <- ESPrcrudedat %>%
...
dscdat <- readRDS("data/derived/descriptive_results_datamap.RDS") 
ESPrcrudedat <- dscdat %>% 
  dplyr::filter(study_id == "ESP1" & breakdown == "region")  %>%
  dplyr::select(c("study_id", "plotdat")) %>%
  tidyr::unnest(cols = "plotdat")
rgnplotdat <- ESPrcrudedat %>%
  dplyr::filter(seromidpt == obsday) %>%
  dplyr::mutate(infxns = popn * seroprev,
                crudeIFR =  cumdeaths/infxns) %>% 
