...
# read in data
#......................
modout <- readRDS("results/ModFits/DNK_age_GTI3_rung50_burn1000_smpl1000.RDS")
```
...
dnkrcrudedat <- dscdat %>% 
  dplyr::filter(study_id == "DNK1" & breakdown == "ageband")  %>%
  dplyr::select(c("study_id", "plotdat")) %>%
  tidyr::unnest(cols = "plotdat")
ageplotdat <- dnkrcrudedat %>%
...
# quick out
jpgsnapshot("figures/DNK_ifr_plot_age.jpg", plot = main_plotObj)
...
serocurvedat %>% 
  ...
  geom_rect(data = SeroPrevObs, aes(xmin = obsdaymin, xmax = obsdaymin, ymin = -Inf, ymax = Inf), 
...
jpgsnapshot("figures/DNK_ppc_plot_age.jpg", plot = ppc_plot_age)
...
# read in data
#......................
modout <- readRDS("results/ModFits/DNK_rgn_GTI3_rung50_burn1000_smpl1000.RDS")
```
...
dscdat <- readRDS("data/derived/descriptive_results_datamap.RDS") 
dnkrcrudedat <- dscdat %>% 
  dplyr::filter(study_id == "DNK1" & breakdown == "region")  %>%
  dplyr::select(c("study_id", "plotdat")) %>%
  tidyr::unnest(cols = "plotdat")
rgnplotdat <- dnkrcrudedat %>%
  dplyr::filter(seromidpt == obsday) %>%
  dplyr::mutate(infxns = popn * seroprev,
                crudeIFR =  cumdeaths/infxns) %>% 
...
# quick out
jpgsnapshot("figures/DNK_ifr_plot_rgn.jpg", plot = main_plotObj)
...
jpgsnapshot("figures/DNK_ppc_plot_rgn.jpg", plot = ppc_plot_rgn)
