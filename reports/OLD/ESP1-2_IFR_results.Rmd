...
# read in data
#......................
modout <- readRDS("results/ModFits/ESP_age_GTI3_rung50_burn1000_smpl1000.RDS")
```
...
ESPcrudedat <- dscdat %>% 
  dplyr::filter(study_id == "ESP1-2" & breakdown == "ageband")  %>%
  dplyr::select(c("study_id", "plotdat")) %>%
  tidyr::unnest(cols = "plotdat")
ageplotdat <- ESPcrudedat %>%
...
plot1 <- ggplot() +
  ...
  geom_point(data = ageplotdat, aes(x = ageband, y = crudeIFR, shape = serodayfct), 
...
# quick out
jpgsnapshot("figures/ESP_ifr_plot_age.jpg", plot = main_plotObj)
...
jpgsnapshot("figures/ESP_ppc_plot_age.jpg", plot = ppc_plot_age)
...
# read in data
#......................
modout <- readRDS("results/ModFits/ESP_rgn_GTI3_rung50_burn1000_smpl1000.RDS")
```
...
ifrs <- dplyr::left_join(rgnkey, ifrs, by = "param") %>% 
  dplyr::mutate(param = factor(param, levels = paste0("ma", 1:17))) %>% 
  dplyr::arrange()
...
dscdat <- readRDS("data/derived/descriptive_results_datamap.RDS") 
ESPcrudedat <- dscdat %>% 
  dplyr::filter(study_id == "ESP1-2" & breakdown == "region")  %>%
  dplyr::select(c("study_id", "plotdat")) %>%
  tidyr::unnest(cols = "plotdat")
rgnplotdat <- ESPcrudedat %>%
  dplyr::filter(seromidpt == obsday) %>%
  dplyr::mutate(infxns = popn * seroprev,
                crudeIFR =  cumdeaths/infxns) %>% 
...
# quick out
jpgsnapshot("figures/ESP_ifr_plot_rgn.jpg", plot = main_plotObj)
...
jpgsnapshot("figures/ESP_ppc_plot_rgn.jpg", plot = ppc_plot_rgn)
