...
quick_sim_sero_diagnostics <- function(modout, spec, sens) {
  ...
  if (modout$inputs$account_seroreversion) {
    ...
    revratechain <- revratechain[[1]][["trace"]] +
      labs(caption = paste0("Prior was Norm+(",  paste(modout$inputs$IFRmodel$paramdf[modout$inputs$IFRmodel$paramdf$name == "sero_rev_rate", c("dsc1", "dsc2")], collapse = ","),
                            ")")) +
      theme(legend.position = "none")
...
tibble::tibble(gelman_mpsrf = gdiag$mpsrf) %>% 
  dplyr::mutate(
    ...
    gelman_mpsrf = kableExtra::cell_spec(gelman_mpsrf, "latex", 
                                         color = ifelse(gelman_mpsrf >= 1.05, "red", "green"),
                                         bold = T,
                                         font_size = 16)) %>% 
...
# plot
labcaption <- paste(strwrap("The blue ribbon is the inferred pserorevalence adjusted for sensitivity and specificity (Rogan-Gladen Correctiion) while the yellow ribbon is the crude seroprevalence. The green and red lines are the simulated true seroprevalence and the observed prevalence (Rogen-Gladen Corrected), respectively. Grey boxes indicate model inputs (serosuvery dates and seroprevalences). N.B. If seroreversion is considered, the true prevalence (green) will also be greater than the observed/inferred prevalences.", width = 180), collapse = "\n") 

pB <- serocurvedat %>% 
  dplyr::left_join(., trueseroprev) %>% 
  ggplot() +
  geom_line(aes(x = ObsDay, y = Crude), alpha = 0.5, color = "#FFD301") + # yellow
  geom_line(aes(x = ObsDay, y = RGCorr), alpha = 0.5, color = "#246BCF") + # blue
  geom_line(aes(x = ObsDay, y = TruePrev), color = "#34A853") + # green
  geom_line(aes(x = ObsDay, y = ObsPrev), color = "#EA4335") +
