...
# regional model
#......................
key <- tibble::tibble(study_id = c("BRA1", "DNK1", "GBR3", "ITA1", "NYS1", "ESP1-2"),
                      nm = c("brazil", "denmark", "gbr", "ita", "nys", "spain"))
rgnmod <- list.files("results/prior_inputs/", pattern = "_reg_age.csv", full.names = T)
rgnmod <- tibble::tibble(nm = stringr::str_split_fixed(basename(rgnmod), "_", n = 2)[,1],
                         path = rgnmod) %>%
