...
# read in region DATA
#...........................................................
region <- readRDS("data/derived/NY/NY_regions.RDS")

#......................
# make agemod for region
#......................
ifr_paramsdf <- make_ma_reparamdf(num_mas = 2)
# save out key for regions
rgnkey <- tibble::tibble(rgn = region$deaths$region[region$deaths$ObsDay == 1], name = ifr_paramsdf$name)
saveRDS(rgnkey, "data/derived/NY/NY_rgn_key.RDS")

knot_paramsdf <- make_splinex_reparamdf(max_xvec = list("name" = "x4", min = 125, init = 131, max = 142, dsc1 = 125, dsc2 = 142),
                                        num_xs = 4)
...
sero_paramsdf <- tibble::tibble(name =  c("sens", "spec", "sero_rate", "sero_day"),
                                min =   c(0.86,     0.97,   10,         88),
                                init =  c(0.88,     0.99,   10,         92),
                                max =   c(0.90,     1.00,   10,         97),
                                dsc1 =  c(8800,     9900,    5,         92),
                                dsc2 =  c(1200,     100,     15,        0.1))
...
pa <- region$seroprev_group$seroprev[region$seroprev_group$region %in% c("New York City, New York", "New York State")]
pa <- pa/sum(pa)
...
agemod$set_IFRparams(paste0("ma", 1:2))
agemod$set_maxMa("ma1")
...
# make "map" for slurm
#......................
map <- tibble::tibble(modelobj = list(agemod), GTI_pow = 3)
