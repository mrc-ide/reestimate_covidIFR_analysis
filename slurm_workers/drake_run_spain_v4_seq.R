...
dir.create("data/param_map/seq/", recursive = T)
...
lapply(split(param_map.fit, 1:nrow(param_map.fit)), function(x){
  saveRDS(x, paste0("data/param_map/seq/",
...
run_MCMC <- function(path) {
  ...
  # out
  dir.create("/proj/ideel/meshnick/users/NickB/Projects/reestimate_covidIFR_analysis/results/drake_seq_test/", recursive = TRUE)
  outpath = paste0("/proj/ideel/meshnick/users/NickB/Projects/reestimate_covidIFR_analysis/results/drake_seq_test/", lvl, "_GTI", GTI_pow, "_rung", rungs, "_burn", burnin, "_smpl", samples, ".RDS")
  out <- list(fit = fit,
...
Sys.sleep(60)
file_param_map <- list.files(path = "data/param_map/seq/",
                             pattern = "*.RDS",
                             full.names = TRUE)
...
make(plan, parallelism = "clustermq", jobs = nrow(file_param_map),
     console_log_file = "drake.log", verbose = 4,
     log_progress = FALSE,
